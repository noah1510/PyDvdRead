    ...
    def dd(inf, outf, blocksize, blocks, label):
        ...
        if os.path.exists(outf):
            ...
            if expectedsize == cursize:
                ...
                # Partial copy
                print("Partial copy: blocksize=%d, blocks=%d, mod=%d" % (
                    blocksize, blocks, blocksize - (cursize % blocksize)))

                # First, need to copy remaining block
                if cursize % blocksize != 0:
                    print("Partial block copy")
                    # Finish remainder of block
                    args = ['dd', 'if=%s' % inf, 'of=%s' % outf, 'bs=1',
                            'count=%d' % (blocksize - (cursize % blocksize)), 'skip=%d' % cursize, 'seek=%d' % cursize]
                    print(" ".join(args))
                    ret = subprocess.call(args)
                    if ret != 0:
                        raise Exception("Failed to partial copy remaining block %d of disc '%s' to drive" % (
                            int(cursize / blocksize) + 1, label))
                ...
                args = ['dd', 'if=%s' % inf, 'of=%s' % outf, 'bs=%d' % blocksize,
                        'count=%d' % (blocks - int(cursize / blocksize)), 'skip=%d' % int(cursize / blocksize),
                        'seek=%d' % int(cursize / blocksize)]
                print(" ".join(args))
                ret = subprocess.call(args)
                if ret != 0:
                    raise Exception("Failed to copy remaining %d blocks (total %d) of disc '%s' to drive" % (
                        blocks - int(cursize / blocksize) + 1, blocks, label))
    ...
    def dvd_GetSize(path):
        ...
        for line in lines:
            ...
            if parts[0] == 'Volume id':
                label = parts[1]
            elif parts[0] == 'Logical block size is':
                blocksize = int(parts[1])
            elif parts[0] == 'Volume size is':
                blocks = int(parts[1])
            else:
                # Don't care
                pass
...
class Title(_dvdread.Title):
    ...
    def __init__(self, DVD, IFONum, TitleNum, AudioClass=None, ChapterClass=None, SubpictureClass=None):
        ...
        _dvdread.Title.__init__(self, DVD, IFONum, TitleNum, AudioClass=AudioClass, ChapterClass=ChapterClass,
                                SubpictureClass=SubpictureClass)
